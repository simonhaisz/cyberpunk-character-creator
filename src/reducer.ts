...
export const reducer: Reducer<State, Action> = (state: State, action: Action): State => {
    switch (action.type) {
        case ActionType.UpdateCharacter: {
            const selectedCharacter = action.data as Character;
        ...
        case ActionType.ClearCharacter: {
            ...
            return { ...state, characters, selectedCharacter: getDefaultCharacter() };
        }
        case ActionType.SelectCharacter: {
            const characterRef = action.data as CharacterRef;
